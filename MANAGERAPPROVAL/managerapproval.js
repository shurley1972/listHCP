...
define(['text!./managerapproval.html', 'text!./workflow.xaml'], function( htmlString, xamlString) { 
	/**
	 * CONSTRUCTOR
	 */
	function managerapproval( params) { 
		var self = this;
		self.params = params;
//debugger;	
		/**
		 * Get parameters used in Approval managerapproval.
		 */
...
//		 
		...
		this.managerapprovalStepEnabled = ko.observable(false);	// Display form if user has design permissions.  Need check from Nikolay for permissions.
...
//
		this.enableui = ko.pureComputed(function() {
			...
			return (this.managerapprovalStepEnabled()) ? true : false;
		}, this);
...
//console.log("obj =" + JSON.stringify(obj));			
			...
			if (step.Current && canEdit) {			
				this.managerapprovalStepEnabled(true);// show Task Form in runtime mode
			};
...
//debugger;			
	...
	}).call(managerapproval.prototype);
	/**
	 ...
	 *  closeonclick
	 */
	managerapproval.prototype.schema = {
		"Params": {			
			"ID": "ManagerApproval",
			"NextID": "",
    ...
    return { viewModel: managerapproval, template: htmlString };
});
